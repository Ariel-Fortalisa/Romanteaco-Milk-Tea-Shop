...
<div class="col-12">
  ...
  <li class="nav-item" role="presentation">
  <button class="nav-link active" id="home-tab" data-toggle="tab" data-target="#home" type="button" role="tab" aria-controls="home" aria-selected="true">In-progress</button>
  </li>
  <li class="nav-item" role="presentation">
...
<div class="tab-content" id="myTabContent">
 ...
 <?php
           ...
           if (isset($_POST['accept_order']) && isset($_POST['order_id'])) {
            ...
            // Update the order status to 3 in the database
            $query = "UPDATE order_list SET status = 3, date = '".date("Y-m-d")."', time = '".date("H:i:s")."' WHERE order_id = $order_id";
            
            if (mysqli_query($db, $query)) {

                $result = $db->query("SELECT * FROM order_item WHERE order_id = $order_id");
                while($row = $result->fetch_assoc()){
                  ...
                  $result2 = $db->query("SELECT * FROM price WHERE product_id = $productId AND size_id = $size");
                  while($row2 = $result2->fetch_assoc()){
                    $sizeId = $row2["size_id"];

                    $result3 = $db->query("SELECT * FROM size_materials WHERE size_id = $sizeId");
                    while($row3 = $result3->fetch_assoc()){
                      $materialId = $row3["material_id"];
                      $qty = $row3["qty"] * $orderQty;
                      $cost = $db->query("SELECT cost FROM materials WHERE material_id = $materialId")->fetch_assoc()["cost"];

                      $db->query("
                        INSERT INTO `material_usage`(
                          `order_item_id`, `material_id`, `qty`, `cost`
                        ) VALUES (
                          '$orderItemId', '$materialId','$qty','$cost'
                        )
                      ");
                    }
...
$query = "SELECT * FROM order_list WHERE status = 2 ORDER BY date ASC, time ASC";
$result = mysqli_query($db, $query);
          ...
          <style>
           ...
           <br>
               ...
               <thead class="th" style="background-color: #e4eccd">
                   ...
                   <tbody>
                       <?php foreach ($productData as $product): ?>
                           ...
                           <td style="vertical-align: baseline;"><svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="#fec400" class="bi bi-circle-fill" viewBox="0 0 16 16">
            ...
            <?php endif; ?>
                            ...
                            <div class="dropdown show d-inline-block widget-dropdown" style="margin-top: -20px;margin-left: 10px; max-width: 5rem">
                                        <a class="dropdown-toggle icon-burger-mini" href="#" role="button" id="dropdown-recent-order1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" data-display="static"></a>
                                        <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-recent-order1">
                                            <li class="dropdown-item">
                                                <a href="#" onclick="acceptOrder(<?php echo $product['order_id']; ?>)"><i class="fas fa-check fa-sm" aria-hidden="true" name="clear"></i>&emsp;Mark as Prepared</a>
                                            </li>
...
function acceptOrder(orderId) {
    ...
    $.ajax({
        type: 'POST',
        url: 'orders_inprogress.php', // Replace with the name of this PHP file
        data: {
            accept_order: true,
            order_id: orderId
        },
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="flex" style="display: flex; gap:.5rem;justify-content: space-between;">
                        ...
                        <ul>
                          <li>
                            <i class="fa-regular fa-clock fa-lg" style="color: #76a004"></i>&ensp;Plenty of time
                          </li>
                        </ul>
                        <ul>
                            <li>
                              <i class="fa-regular fa-clock fa-lg" style="color: #fe5461"></i>&ensp;Overdue
                            </li>
                          </ul>
                        </div>
                        <div class="inputBox" style="position: relative; width: 50%">
                        ...
                        <h6 style="padding-bottom: .4rem; color: #fff">Estimated Time Status:</h6>
                          <ul>
                            <li>
                              <i class="fa-regular fa-clock fa-lg" style="color: #fec400"></i>&ensp;10 minutes left 
                            </li>
                          </ul>
                        </div>
                      </div>
