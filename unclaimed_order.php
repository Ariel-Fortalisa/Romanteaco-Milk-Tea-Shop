...
<div class="col-12">
    <div class="card card-default">
        ...
        <?php
            ...
            if (isset($_POST['claimed_order']) && isset($_POST['order_id'])) {
...
$query = "SELECT * FROM order_list WHERE status = 11 ORDER BY date ASC, time ASC";
$result = mysqli_query($db, $query);
                    ...
                    <tbody>
                        <?php foreach ($productData as $product): ?>
                            <tr style="<?=empty($product['estimated_arrival']) ? "" : "background-color: #e0f2ff"?>">
                                <td style="vertical-align: baseline;"><svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="gray" class="bi bi-circle-fill" viewBox="0 0 16 16">
                            ...
                            <td style="vertical-align: baseline; text-align: center"><?php echo empty($product['estimated_arrival']) ? "N/A" : date('h:i A', strtotime($product['estimated_arrival'])).'&ensp;<i class="fa-regular fa-clock" style="color: #fe5461"></i>'; ?></td>
                                    ...
                                    <div class="dropdown show d-inline-block widget-dropdown" style="margin-top: -20px;margin-left: 10px">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-recent-order1">
                                            <li class="dropdown-item">
                                                <a href="#" onclick="claimedOrder(<?php echo $product['order_id']; ?>)"><i class="fas fa-check fa-sm" aria-hidden="true" name="clear"></i>&emsp;Mark as claimed</a>
                                            </li>
                                            <li class="dropdown-item">
                                                <a href="#" onclick="restoreOrder(<?php echo $product['order_id']; ?>)"><i class="fas fa-clock-rotate-left fa-sm" aria-hidden="true" name="clear"></i>&emsp;Restore</a>
                                            </li>
                                        </ul>
                                    </div>
                                </td>
...
<script>
function claimedOrder(orderId) {
    // Send an AJAX request to update the order status
    $.ajax({
        type: 'POST',
        url: 'unclaimed_order.php', // Replace with the name of this PHP file
        data: {
            claimed_order: true,
            order_id: orderId
        },
