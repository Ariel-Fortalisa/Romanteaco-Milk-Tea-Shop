...
function deleteMaterial($db, $material_id) {
  // Retrieve the details of the material before deletion
  $selectQuery = "SELECT name, price, quantity, measurement FROM add_ons1 WHERE add_id = $material_id";
  $result = mysqli_query($db, $selectQuery);
  $row = mysqli_fetch_assoc($result);

  // Delete the material from the 'add_ons1' table
  $deleteMaterialQuery = "DELETE FROM add_ons1 WHERE add_id = $material_id";

  // Perform the deletion operation
  if (mysqli_query($db, $deleteMaterialQuery)) {
      // Log the deletion activity
      $deleteActivity = "Admin permanently deleted {$row['quantity']} pack of {$row['name']} with a price of {$row['price']}, and a measurement of {$row['measurement']}";

      // Insert the deletion activity log into the "activity_logs_addons" table
      $activityLogsSql = "INSERT INTO activity_logs_addons (activity, date, time) VALUES (?,?,?)";
...
// Check if the "add_id" parameter is present in the request
if (isset($_POST['add_id'])) {
  $material_id = $_POST['add_id'];
  ...
  if (isset($_POST['confirmDelete']) && $_POST['confirmDelete'] === 'yes') {
      ...
      if (deleteMaterial($db, $material_id)) {
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  if (isset($_POST["addMaterial"])) {
      // Retrieve form data
      $materialName = $_POST["name"]; // Updated to "name" field
      $price = $_POST["price"];
      ...
      // SQL query to insert data into the "add_ons1" table
      $sql = "INSERT INTO add_ons1 (name, price, quantity, cost, measurement, expiration_date, expiration_status) VALUES (?, ?, ?, ?, ?, ?, ?)";
      $stmt = mysqli_prepare($db, $sql);

      if ($stmt) {
          // Bind parameters and execute the statement
          $goodStatusPlaceholder = "GOOD";
          mysqli_stmt_bind_param($stmt, "sdddsss", $materialName, $price, $quantity, $cost, $measurement, $expirationDate, $goodStatusPlaceholder);

          if (mysqli_stmt_execute($stmt)) {
              // Data inserted successfully

              // Log the activity
              $activity = "Admin added $quantity pack of $materialName with a price of $price, and a measurement of $measurement";
              $activityLogsSql = "INSERT INTO activity_logs_addons (activity, date, time) VALUES (?,?,?)";
              $activityLogsStmt = mysqli_prepare($db, $activityLogsSql);
              ...
              echo "<script>window.location = 'add-ons.php';</script>";
...
function updateMaterial($db, $addId, $materialName, $price, $quantity, $measurement, $expirationDate) {
    $expDt = str_replace("'", "", $expirationDate);

  $materialName = mysqli_real_escape_string($db, $materialName);

  // Retrieve the existing material data for logging
  $queryGetMaterialData = "SELECT name, price, quantity, measurement, expiration_date FROM add_ons1 WHERE add_id = $addId";
  $resultGetMaterialData = mysqli_query($db, $queryGetMaterialData);
  $rowGetMaterialData = mysqli_fetch_assoc($resultGetMaterialData);
  $existingMaterialName = $rowGetMaterialData['name'];
  $existingPrice = $rowGetMaterialData['price'];
  $existingQuantity = $rowGetMaterialData['quantity'];
  $existingMeasurement = $rowGetMaterialData['measurement'];
  ...
  $query = "UPDATE add_ons1 SET name = '$materialName', price = '$price', quantity = '$quantity', measurement = '$measurement', expiration_date = $expirationDate WHERE add_id = $addId";
  $result = mysqli_query($db, $query);
  ...
  if ($result) {
      ...
      if ($existingPrice !== $price) {
          // Admin updated the price
          $updateActivity .= "Admin updated the price of $existingMaterialName from $existingPrice to $price";
      }
    ...
    if ($existingMeasurement !== $measurement) {
        ...
        $updateActivity .= "Admin updated the description of $materialName from $existingMeasurement to $measurement";
    }
    ...
    $ex = $existingExpirationDate == "" ? "NULL" : $existingExpirationDate;
      ...
      if (!empty($updateActivity)) {
          // Insert the update activity log into the "activity_logs_addons" table
          $activityLogsSql = "INSERT INTO activity_logs_addons (activity, date, time) VALUES (?,?,?)";
          $activityLogsStmt = mysqli_prepare($db, $activityLogsSql);
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if(isset($_POST["editAddonsSubmit"])){
        $add_id = $_POST["add_id"]; // Changed from $material_id
        $material_name = $_POST["material_name"]; // Changed from $material_name
        $price = $_POST["price"]; // Added for the price field
        $quantity = $_POST["quantity"];
        $measurement = $_POST["measurement"];
        $expiration_date = !empty($_POST["expiration_date"]) ? "'".$_POST["expiration_date"]."'" : "NULL";

        if (updateMaterial($db, $add_id, $material_name, $price, $quantity, $measurement, $expiration_date)) {
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $updateQuery1 = "UPDATE add_ons1 SET status = '$newStatus' WHERE add_id = {$row['add_id']}";
    mysqli_query($db, $updateQuery1);

    // Update the 'status' column in the 'price' table (assuming there is a 'price' table)
    $updateQuery2 = "UPDATE add_ons SET status = '$newStatus' WHERE add_id = {$row['add_id']}";
...
$productData = array();
$query = "SELECT * FROM add_ons1"; // Refresh the original query
$result = mysqli_query($db, $query);
while ($row = mysqli_fetch_assoc($result)) {
    $productData[] = $row;
}
            ...
            <div class="modal-body" style="padding-right: 0;">
                <div style="max-height: 400px; overflow-y: auto;">
                    <ul>
                        ...
                        $query = "SELECT * FROM activity_logs_addons ORDER BY date DESC, time DESC";
                        $result = mysqli_query($db, $query);
...
$sql = "UPDATE add_ons1 SET expiration_status = ? WHERE add_id = ?";
$stmt = $db->prepare($sql);
$stmt->execute([$expirationStatus, $product['add_id']]);
    ...
    <div class="dropdown show d-inline-block widget-dropdown" style="margin-top: -10px;">
        ...
        <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-recent-order1">
            <li class="dropdown-item">
                <a href="#" data-toggle="modal" data-target="#editModal_<?php echo $product['add_id']; ?>"><i class="fas fa-edit fa-sm" aria-hidden="true" name="clear"></i>&emsp;Edit</a>
            </li>
            <li class="dropdown-item">
                <!-- Trigger the Bootstrap modal -->
                <a href="#" data-toggle="modal" data-target="#confirmDeleteModal_<?php echo $product['add_id']; ?>"><i class="fas fa-trash fa-sm" aria-hidden="true" name="clear"></i>&emsp;Remove</a>
            </li>
        </ul>
    </div>
</td>
...
<?php foreach ($productData as $product): ?>
    <div class="modal fade" id="confirmDeleteModal_<?php echo $product['add_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="confirmDeleteModalLabel_<?php echo $product['add_id']; ?>" aria-hidden="true">
        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="confirmDeleteModalLabel_<?php echo $product['add_id']; ?>"><svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="orange" class="bi bi-exclamation-triangle-fill" viewBox="0 0 16 16">
  <path d="M8.982 1.566a1.13 1.13 0 0 0-1.96 0L.165 13.233c-.457.778.091 1.767.98 1.767h13.713c.889 0 1.438-.99.98-1.767L8.982 1.566zM8 5c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995A.905.905 0 0 1 8 5zm.002 6a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"/>
                ...
                <div class="modal-body">
                    <p>Are you sure you want to delete this add ons?</p><br>
                    <p style="font-size: 13px;">
                    Warning: Clicking "Yes" will result in permanent deletion, and this action cannot be undone.
                    </p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">No</button>
                    <form method="POST">
                        <input type="hidden" name="add_id" value="<?php echo $product['add_id']; ?>">
...
<div class="modal fade" id="addMaterialModal" tabindex="-1" role="dialog" aria-labelledby="addMaterialModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addMaterialModalLabel">Add Add-ons</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                ...
                <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                    ...
                    <div class="form-group">
                        <label for="addMaterialName" style="text-transform: none">Add-ons Name</label>
                        <input type="text" class="form-control" id="addMaterialName" name="name" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control" id="addExpirationDate" name="date">
                    </div>

                    <div class="text-danger"><?php if (isset($addErrorMessage)) echo $addErrorMessage; ?></div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-primary" name="addMaterial">Add Add-ons</button>
                    </div>
                </form>
            </div>
        </div>
 ...
 <!-- edit modal -->
<?php foreach ($productData as $product): ?>
    <div class="modal fade" id="editModal_<?php echo $product['add_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editModalLabel">Add-ons Details</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    ...
                    <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="editMaterialName" name="material_name" value="<?php echo $product['name']; ?>">
                        </div>
                        <div class="form-group">
                            <label for="editPrice">Price</label>
                            <input type="number" class="form-control" id="editPrice" name="price" value="<?php echo $product['price']; ?>">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                            <button type="submit" class="btn btn-primary" name="editAddonsSubmit">Save changes</button>
                        </div>
                    </form>
                </div>
            </div>
