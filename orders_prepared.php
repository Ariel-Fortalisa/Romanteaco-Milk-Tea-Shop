    ...
    <div class="card card-default">
        ...
        <div class="card-body">
            ...
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="home-tab" data-toggle="tab" data-target="#home" type="button" role="tab" aria-controls="home" aria-selected="true">Prepared</button>
                </li> 
                </li>
                <li class="nav-item" role="presentation">
...
<div class="tab-content" id="myTabContent">
 ...
 <?php
           ...
           if (isset($_POST['accept_order']) && isset($_POST['order_id'])) {
            ...
            // Update the order status to 4 in the database
            $query = "UPDATE order_list SET status = 4, date = '".date("Y-m-d")."', time = '".date("H:i:s")."' WHERE order_id = $order_id";
        ...
        if (isset($_POST['unclaimed_order']) && isset($_POST['order_id'])) {
          $order_id = $_POST['order_id'];
      
          // Update the order status to 2 in the database
          $query = "UPDATE order_list SET status = 11, date = '".date("Y-m-d")."', time = '".date("H:i:s")."' WHERE order_id = $order_id";
...
$query = "SELECT * FROM order_list WHERE status = 3 ORDER BY date ASC, time ASC";
$result = mysqli_query($db, $query);
               ...
               <thead class="th" style="background-color: #e4eccd">
                   ...
                   <tbody>
                       <?php foreach ($productData as $product): ?>
                           ...
                           <td style="vertical-align: baseline;"><svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="#f58216" class="bi bi-circle-fill" viewBox="0 0 16 16">
                            ...
                            <div class="dropdown show d-inline-block widget-dropdown" style="margin-top: -20px; margin-left: 10px">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-recent-order1">
                                            <li class="dropdown-item">
                                                <a href="#" onclick="acceptOrder(<?php echo $product['order_id']; ?>)"><i class="fas fa-check fa-sm" aria-hidden="true" name="clear"></i>&emsp;Mark as Done</a>
                                            </li>
                                            <?php
                                              if(!empty($product['estimated_arrival'])){
                                            ?>
                                            <li class="dropdown-item">
                                                <a href="#" onclick="unclaimedOrder(<?php echo $product['order_id']; ?>)"><i class="fas fa-xmark fa-sm" aria-hidden="true" name="clear"></i>&emsp;Unclaimed</a>
                                            </li>
                                            <?php
                                            }
                                            ?>
...
function acceptOrder(orderId) {
    ...
    $.ajax({
        type: 'POST',
        url: 'orders_prepared.php', // Replace with the name of this PHP file
        data: {
            accept_order: true,
            order_id: orderId
        },
...
function unclaimedOrder(orderId) {
    // Send an AJAX request to update the order status
    $.ajax({
        type: 'POST',
        url: 'orders_prepared.php', // Replace with the name of this PHP file
        data: {
            unclaimed_order: true,
            order_id: orderId
        },
        success: function(response) {
