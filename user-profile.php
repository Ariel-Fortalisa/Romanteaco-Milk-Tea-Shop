  ...
  <div class="row no-gutters">
    ...
    if ($result) {
        ...
        if (!empty($productData['image'])) {
            ...
            echo '<img src="' . $imageURL . '" alt="Product Image" style="height: 100px; width: 100px;">';
        } else {
            echo 'No image found.';
        }
...
$query = "SELECT fname, lname, email, phone FROM admin_user";
$result = mysqli_query($db, $query);
...
<div class="card-body">
    <?php foreach ($userData as $user) { ?>
        <h3 class="py-2 text-dark"><?php echo $user['fname'] . ' ' . $user['lname']; ?></h3>
        <p style="padding: 0px 10px"><?php echo $user['email']; ?></p>
    ...
    <div class="col-lg-8 col-xl-9">
      <div class="profile-content-right profile-right-spacing py-5">
        <ul class="nav nav-tabs px-3 px-xl-5 nav-style-border" id="myTab" role="tablist">
          ...
          <li class="nav-item">
            <a class="nav-link active" id="settings-tab" data-toggle="tab" href="#settings" role="tab" aria-controls="settings" aria-selected="false"><h4 style="color: #76a004;">Account Settings</h4></a>
          </li>
        </ul>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["updateProfile"])) {
  ...
  if (!empty($_FILES['image']['name'])) {
      ...
      $updateQuery = "UPDATE admin_user SET fname=?, lname=?, email=?, phone=?, image=? WHERE id=?";
      $stmt = mysqli_prepare($db, $updateQuery);

      // Bind parameters and execute the statement
      mysqli_stmt_bind_param($stmt, "sssssi", $fname, $lname, $email, $phone, $image_path, $id);
  } else {
      // Update only fname, lname, email, phone, and hashed password when no image is uploaded
      $updateQuery = "UPDATE admin_user SET fname=?, lname=?, email=?, phone=? WHERE id=?";
      $stmt = mysqli_prepare($db, $updateQuery);

      // Bind parameters and execute the statement
      mysqli_stmt_bind_param($stmt, "ssssi", $fname, $lname, $email, $phone, $id);
  }

  if($password !== ""){
    // Update only password
    $updateQuery = "UPDATE admin_user SET password=? WHERE id=?";
    $stmt = mysqli_prepare($db, $updateQuery);

    // Bind parameters and execute the statement
    mysqli_stmt_bind_param($stmt, "si", $encpass, $id);
  }
...
$query = "SELECT id, fname, lname, email, phone, password FROM admin_user WHERE id=?";
    ...
    <div class="tab-pane-content mt-5" style="z-index: 10;">
        <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
            ...
            <div class="row mb-2">
                <div class="col-lg-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="firstName" name="firstName" placeholder="Update Firstname"
                ...
                <div class="col-lg-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="lastName" name="lastName" placeholder="Update Lastname"
            ...
            <div class="form-group mb-4">
                <label for="phone">Phone</label><span class="text-left" style="font-size:.9rem;position:absolute;margin-left: 1rem"><i class="text-muted">*e.g. 09XXXXXXXXX</i></span>
                <input type="number" class="form-control" 
                oninput="javascript: if (this.value.length > this.maxLength) 
                this.value = this.value.slice(0, this.maxLength);" maxlength="11" 
                id="phone" name="phone" placeholder="Update Phone Number"
            ...
            <div class="d-flex justify-content-end mt-5">
                <button type="submit" class="btn mb-2 btn-pill" name="updateProfile" style="background-color: #76a004; color: #fff; border-radius: .3rem">Update Profile</button>
            </div>
        </form>
    </div>
</div>
